function getModule()
{
	...
	{
		v1[i] = new Option(metaArray[i]["modulename"],metaArray[i]["modulename"])
	}
}
...
function getKpiname()
{
	...
	var kpis = metaArray[k]["kpis"]
	v1.length = 0
	for(var x in kpis)
	{
		var opt = new Option(x,x)
		v1.appendChild(opt)
	}
	getViewname()
}
...
function getViewname()
{
	...
	var kpi = document.form1.kpiname.value
	var k  = document.form1.module.selectedIndex
	var views = metaArray[k]["kpis"][kpi]["views"]
	v1.length = 1
	for(var x in views)
	{
		var opt = new Option(x,x)
	...
	v1[0] = new Option("total(average)","total")
	v1.value = "total"
	getDims()
}
...
function getDims()
{
	...
	var kpi = document.form1.kpiname.value
	var view= document.form1.viewname.value
	var v1  = document.form1.views
	v1.length = 1
	if( view == "total" )
		return
	var dims= metaArray[m]["kpis"][kpi]["views"][view]["dims"]
	for(var x in dims)
	{
		var opt = new Option(x,x)
		v1.appendChild(opt)
...
function getKpiValue(kpis,kpi,view,dim)
{
	if(view == "total")
		return getJsonValue(kpis,kpi,"kpi_total")
	else
		return getJsonValue(kpis,kpi,"views",view,"dims",dim)
}
...
function getData()
{
   ...
   var kpi = document.form1.kpiname.value
   var view= document.form1.viewname.value
   var dim = document.form1.views.value
   ...
   for(var i=0;i<str1.length;i++)
   {
      ...
      if(d != "total" && c == "all")
      {
         for(view1 in dims)
         {
            mydata[i][view1] = getKpiValue(kpis1,kpi,view,view1)
      ...
      {
         mydata[i][myykey[0]] = getKpiValue(kpis1,kpi,view,dim)
   ...
   for(var i=0;i<str2.length;i++)
   {
      ...
      mydata[i][myykey[1]] = getKpiValue(kpis2,kpi,view,dim)
